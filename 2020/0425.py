...
for k,t in enumerate(np.linspace(0,2*pi,180)):
    ...
    for j,z in enumerate(np.linspace(0,1,30)):
        plt.scatter([z*cos(x)*(sum([h[i]*cos(f[i]*x+t) for i in range(10)])+15) for x in X],
                    [z*sin(x)*(sum([h[i]*sin(f[i]*x+t) for i in range(10)])+15) for x in X],
                    color=colors[j],
                    s=32,
                    lw=0.8)
    p = plt.savefig(f'C:/Users/Alejandro/Pictures/RandomPlots/25042020/plot{k}.PNG',facecolor='black')