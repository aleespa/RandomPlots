...
p = plt.axis('off')
p = plt.scatter(random1,
                random2,
                s=random3,
                color='white',zorder=n**2+10)
for i in range(n):
    for j in range(n):
        if np.random.choice([False, True],p=[0.99,0.01]):
            plt.plot([random1[i],random1[j]],[random2[i],random2[j]],alpha=0.3,lw=1,color=plt.cm.inferno(np.random.uniform(0,1)))
