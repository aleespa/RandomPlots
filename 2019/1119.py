...
for j in np.linspace(0,12*pi,210):
    ...
    for i in np.linspace(-12,12,60):
        p= plt.plot(np.linspace(0,10),[sin(x+j)+i for x in np.linspace(0,10)],color=CC[n],lw=5)
        n+=1
    p = plt.savefig(f'C:/Users/Alejandro/Pictures/RandomPlots/19112019/plor{m}.png',facecolor='black')
    m+=1