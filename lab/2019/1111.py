...
def generate():
    ...
    for x in range(30):
        for y in range(30):
            ...
            ax.plot([p1[0],p1[0]+r2],[p1[1],p1[1]+r1],color = np.random.choice(colors1))
            ax.plot([p1[0],p1[0]-r1],[p1[1],p1[1]-r2],color = np.random.choice(colors1))
    ...
    for x in range(30):
        for y in range(30):
            ...
            for q in  np.random.choice([-1,0,1],2,p=[0.2,0.6,0.2]):
                ax.plot([p1[0],p1[0]],[p1[1],p1[1]+q],color = np.random.choice(colors2))
            for q in  np.random.choice([-1,0,1],2,p=[0.2,0.6,0.2]):
                ax.plot([p1[0],p1[0]+q],[p1[1],p1[1]],color = np.random.choice(colors2))
    name = ''.join(random.choices(string.ascii_letters + string.digits, k=8))
    logger.info(f"{name}.png Saved")
    fig.savefig(f'outputs/{filename}/{name}.png', facecolor='k')
    plt.close()
