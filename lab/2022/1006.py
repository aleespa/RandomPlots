...
def plot_1():
    ...
    Z_initial = np.concatenate(
        (
            8 * np.cos(np.linspace(0, 2 * pi, m + 1)).reshape(-1, 1)[:-1] + 2,
            8 * np.sin(np.linspace(0, 2 * pi, m + 1)).reshape(-1, 1)[:-1] + 2,
        ),
        axis=1,
    ).T
    Z = np.zeros((2, m, n))
    ...
    Z[:, :, 0] = Z_initial
    Z_smooth[:, :, :initial_anchor] = np.repeat(
        Z_initial[:, :, np.newaxis], initial_anchor, axis=2
    )
    for i in range(n - 1):
    ...
    for frame in range(233, n_frames):
        ...
        for j in range(m):
            plt.scatter(
                Z_smooth[0, j, (min(n_frames - 1, frame + 100)) * ppf + 1],
                Z_smooth[1, j, (min(n_frames - 1, frame + 100)) * ppf + 1],
                color='r',
                zorder=2,
                s=50,
            )
            ...
            ax.plot(
                Z_smooth[0, j, frame * ppf : (min(n_frames - 1, frame + 100)) * ppf],
...
def plot_3():
    ...
    for k, r in enumerate(np.linspace(25, 30, 35)):
        m_4 = int(m / 4)
        Z_initial = np.concatenate(
            (np.linspace(-6, 6, m_4).reshape(-1, 1), np.zeros(m_4).reshape(-1, 1) - 6),
            axis=1,
        ).T
        Z_initial = np.concatenate(
            (
                ...
                np.concatenate(
                    (
                        np.linspace(-6, 6, m_4).reshape(-1, 1),
                        np.zeros(m_4).reshape(-1, 1) + 6,
                    ),
                    axis=1,
                ).T,
            ),
